// Calculo numerico: Tema 3 / taylor2.cpp
...
int main()
{
    ...
    while (fabs(termino64) > epsilon)
    {
        ...
        float error_rel_32 = fabs((exp(-x) - serie32) / exp(-x)); // Error relativo
        double error_rel_64 = fabs((exp(-x) - serie64) / exp(-x));

        cout << n << '\t' << termino32 << '\t' << termino64 << '\t' << serie32 << '\t' << serie64 << '\t' << error_rel_32 << '\t' << error_rel_64 << endl;
    }
